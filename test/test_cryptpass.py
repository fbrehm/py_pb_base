...
class TestCryptPass(PbBaseTestcase):
    ...
    def test_shadowcrypt_valid(self):
        ...
        for algo in ('crypt', 'md5', 'sha256', 'sha512'):
            ...
            self.assertGreater(
                len(crypted), length, ((
                    "The encrypted password must be longer than the length "
                    "of salt %d.") % (length)))
            salt_pattern = r'^%s' % (re.escape(salt2use))
            self.assertRegexpMatches(
                crypted, salt_pattern, ((
                    "The salt %r must be embedded at the beginning of the ecrypted "
            ...
            self.assertEqual(
                crypted, crypted2, ((
                    "Encrypted password with simple salt %r (-> %r) must be "
                    "identic with the " +
